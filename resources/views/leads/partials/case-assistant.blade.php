...
>
    ...
    <div class="case-assistant-body">
        ...
        <section class="case-assistant-history-wrap" aria-labelledby="caseAssistantHistoryHeading">
            ...
            <div id="caseAssistantHistory" class="case-assistant-history"><div class="case-assistant-muted">Loading previous conversation…</div></div>
        </section>
    </div>
    <form class="case-assistant-composer" id="caseAssistantComposer">
        ...
        <textarea id="caseAssistantInput" class="case-assistant-input" rows="3" placeholder="Ask about this case, start an I&E, or teach Jinx a new rule…"></textarea>
        <div class="case-assistant-composer-row"><span class="case-assistant-muted" id="caseAssistantStatus">Connecting…</span><button type="submit" id="caseAssistantSend" class="case-assistant-send" style="cursor:pointer; background:#2563eb; color:#fff;">Send</button></div>
    </form>
</aside>
<style>
...
(() => {
    ...
    function renderMessage(message) { const role=message.role==='user'?'user':'assistant'; const label=role==='user'?'You':'Jinx'; return `<div class="jinx-assistant-message jinx-assistant-message-${role}"><span class="jinx-assistant-message-label">${label}</span>${escapeHtml(message.content)}</div>`; }
    function appendMessage(message) { if(history.querySelector('[data-empty="1"]')) history.innerHTML=''; history.insertAdjacentHTML('beforeend',renderMessage(message)); history.scrollTop=history.scrollHeight; }
    function setBusy(busy,text=null) { input.disabled=busy; sendButton.disabled=busy; sendButton.style.opacity=busy?'.6':'1'; if(text!==null)status.textContent=text; }
    async function bootstrap() {
        try {
            const response=await fetch(`/assistant/lead/${leadId}`,{headers:{'Accept':'application/json'},credentials:'same-origin'}); const data=await response.json();
    ...
    composer.addEventListener('submit',async(event)=>{
        ...
        appendMessage({role:'user',content:message}); input.value=''; setBusy(true,'Jinx is thinking…');
        try {
            const response=await fetch(`/assistant/lead/${leadId}/message`,{method:'POST',credentials:'same-origin',headers:{'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':csrf},body:JSON.stringify({message})});
        ...
        } catch(error){appendMessage({role:'assistant',content:`I couldn't respond: ${error.message}`});status.textContent='Error';}
    ...
    resetButton.addEventListener('click',async()=>{
        ...
        try{const response=await fetch(`/assistant/lead/${leadId}/reset`,{method:'POST',credentials:'same-origin',headers:{'Accept':'application/json','X-CSRF-TOKEN':csrf}});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.message||'Unable to reset chat');history.innerHTML='<div class="case-assistant-muted" data-empty="1">New conversation started. Shared Jinx knowledge is still available.</div>';status.textContent='Ready';}
        catch(error){status.textContent=error.message;}finally{setBusy(false);input.focus();}
    });
    bootstrap();
})();
</script>
