...
<div class="portal-card" style="max-width:560px; margin:40px auto 0; text-align:center;">
    ...
    <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:10px; margin-top:14px;">
        @if(!blank($portalWhatsAppUrl))
            <a href="{{ $portalWhatsAppUrl }}" style="display:inline-block; text-decoration:none; padding:10px 14px; border-radius:10px; background:#16a34a; color:#ffffff; font-size:14px; font-weight:700;">Message us on WhatsApp</a>
        @endif
        @if(!blank($portalCallUrl))
            <a href="{{ $portalCallUrl }}" style="display:inline-block; text-decoration:none; padding:10px 14px; border-radius:10px; background:#1d4ed8; color:#ffffff; font-size:14px; font-weight:700;">Call us</a>
        @endif
    </div>
</div>
@endsection
