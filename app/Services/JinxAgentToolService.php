...
class JinxAgentToolService
{
    ...
    { $l=Lead::findOrFail((int)$a['lead_id']);$status=(string)$a['status'];if(!in_array($status,Lead::WIP_STATUSES,true))throw new RuntimeException('Invalid WIP status.');$reason=trim((string)($a['reason']??''));if($status==='Dead'&&$reason==='')throw new RuntimeException('A reason is required when marking a case Dead.');$old=$l->wip_status;$l->update(['wip_status'=>$status,'dead_reason'=>$status==='Dead'?$reason:null]);$removed=0;if($l->vicidial_lead_id&&in_array($status,['Collecting Docs','Callback Set','DMP Transfer','Ready to Refer','SIP Booked','IVA Verified','DMP Verified','Lost Contact','Dead'],true)){$c=(string)config('services.vicidial.db_connection','asterisk');$removed=DB::connection($c)->table('vicidial_hopper')->where('lead_id',(int)$l->vicidial_lead_id)->delete();}return ['success'=>true,'lead_id'=>$l->id,'old_status'=>$old,'new_status'=>$status,'dead_reason'=>$l->dead_reason,'hopper_rows_removed'=>$removed]; }
