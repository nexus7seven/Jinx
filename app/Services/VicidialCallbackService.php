...
class VicidialCallbackService
{
    ...
    public function cancel(Lead $lead, string $reason = ''): array
    {
        ...
        return DB::connection($connection)->transaction(function () use ($connection, $leadId, $reason, $lead) {
            $diallerLead = DB::connection($connection)->table('vicidial_list')->where('lead_id', $leadId)->lockForUpdate()->first();
            if (! $diallerLead) throw new RuntimeException('VICIdial lead not found.');

            $active = DB::connection($connection)->table('vicidial_callbacks')->where('lead_id', $leadId)->whereIn('status', ['ACTIVE', 'LIVE'])->get(['callback_id']);
            if ($active->isEmpty()) return ['cancelled' => 0, 'callback_ids' => [], 'vicidial_lead_id' => $leadId, 'reason' => trim($reason), 'jinx_status' => $lead->fresh()->wip_status];
            ...
            DB::connection($connection)->table('vicidial_hopper')->where('lead_id', $leadId)->delete();
            if ($lead->wip_status === 'Callback Set') $lead->update(['wip_status' => 'Collecting Docs']);

            return ['cancelled' => $active->count(), 'callback_ids' => $active->pluck('callback_id')->map(fn ($id) => (int) $id)->all(), 'vicidial_lead_id' => $leadId, 'reason' => trim($reason), 'jinx_status' => $lead->fresh()->wip_status];
        });
    }
