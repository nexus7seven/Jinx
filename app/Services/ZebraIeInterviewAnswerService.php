...
class ZebraIeInterviewAnswerService
{
     ...
     * A valid answer is persisted against that exact checkpoint before the next state is chosen.
     */
    public function currentStep(array $facts): ?array
    {
        if (!$this->has($facts, 'calculation.target_di')) return $this->step('calculation.target_di', 'money', 'What is the target DI?');
        if (!$this->has($facts, 'income.client_salary')) return $this->step('income.client_salary', 'money', "What is the client's monthly take-home salary?");
        if (!$this->has($facts, 'household.partner_exists')) return $this->step('household.partner_exists', 'yes_no', 'Does the client have a partner?');
        if ($this->bool($facts, 'household.partner_exists') === true && !$this->has($facts, 'income.partner_salary')) return $this->step('income.partner_salary', 'money', "What is the partner's monthly take-home salary?");
        if (!$this->has($facts, 'household.children_count')) return $this->step('household.children_count', 'children_count', 'How many children live with the client?');
        ...
        if (!$this->has($facts, 'income.universal_credit')) return $this->step('income.universal_credit', 'money', "What is the client's monthly Universal Credit? Enter 0 if none.");
        ...
        if (!$this->has($facts, 'housing.rent_mortgage')) return $this->step('housing.rent_mortgage', 'money', 'What is the monthly rent or mortgage?');
        if (!$this->has($facts, 'housing.council_tax')) return $this->step('housing.council_tax', 'money', 'What is the monthly Council Tax?');
        if (!$this->has($facts, 'transport.client.mode')) return $this->step('transport.client.mode', 'transport', 'Does the client have a car or use public transport?');
        if ($this->isCar($facts['transport.client.mode'] ?? null) && !$this->has($facts, 'transport.client.car_insurance')) return $this->step('transport.client.car_insurance', 'money', "What is the client's monthly car insurance?");

        if ($this->bool($facts, 'household.partner_exists') === true) {
            if (!$this->has($facts, 'transport.partner.mode')) return $this->step('transport.partner.mode', 'transport', 'Does the partner have a car or use public transport?');
            if ($this->isCar($facts['transport.partner.mode'] ?? null) && !$this->has($facts, 'transport.partner.car_insurance')) return $this->step('transport.partner.car_insurance', 'money', "What is the partner's monthly car insurance?");
        }

        if (!$this->has($facts, 'other.childcare')) return $this->step('other.childcare', 'money', 'Does the client have any monthly childcare costs? Enter 0 if none.');
        if (!$this->has($facts, 'other.maintenance_paid')) return $this->step('other.maintenance_paid', 'money', 'Does the client pay monthly maintenance for children who do not live with them? Enter 0 if none.');
    ...
    {
        if (!$this->has($facts, 'income.client_salary')) return false;
        if (!$this->has($facts, 'household.partner_exists')) return false;
        if ($this->bool($facts, 'household.partner_exists') === true && !$this->has($facts, 'income.partner_salary')) return false;
        if (!$this->has($facts, 'household.children_count')) return false;
        $children = (int) ($facts['household.children_count'] ?? 0);
        $ages = $facts['household.children_ages'] ?? [];
        if ($children > 0 && (!is_array($ages) || count($ages) !== $children)) return false;
        if (!$this->has($facts, 'income.universal_credit')) return false;
    ...
    public function extract(Lead $lead, array $facts, string $message): array
    {
        ...
        return $this->parseStep($step, $facts, $message);
    }
