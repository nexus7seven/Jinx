...
class JinxAgentIvaService
{
    ...
    public function facts(Lead $lead): array
    {
        ...
        $put('housing.type',data_get($s,'facts.housing_type'));foreach(['rent_mortgage','council_tax','tv_licence'] as $k)$put('housing.'.$k,data_get($s,'expenditure.housing.'.$k));
        ...
        $put('transport.household.breakdown_cover',data_get($s,'expenditure.transport.household.breakdown_cover'));
        foreach(['childcare','maintenance_paid','dla_care','pip_care','student_offset','adult_care','prescriptions','dentistry','other'] as $k)$put('other.'.$k,data_get($s,'expenditure.other.'.$k));
        return $f;
    }
